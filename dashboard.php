...
function leerPagina() {
    ...
    const u = document.querySelector('.user-chip')?.textContent.trim() || '';
    const stats = [...document.querySelectorAll('.stat-card')].map(c => c.getAttribute('aria-label') || c.querySelector('.stat-label')?.textContent + ': ' + c.querySelector('.stat-num')?.textContent).filter(Boolean);
    const alertas = [...document.querySelectorAll('.alert-cero, .alert-strip')].map(a => a.querySelector('h4')?.textContent).filter(Boolean);
    const frases = [
        'Bienvenido al Dashboard de OfficeStock. Usuario: ' + u + '.',
        'Resumen del inventario: ' + stats.slice(0,4).join('. ') + '.',
    ];
    if (alertas.length) frases.push('Alertas activas: ' + alertas.join('. '));
    frases.push('Usa los accesos rápidos para navegar a Productos, Inventario, Usuarios, Categorías o Salidas.');
    frases.push('Fin del resumen.');
